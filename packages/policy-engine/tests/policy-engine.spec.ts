...
describe('policy-engine rules', () => {
  it('insurance rule fails for expired insurance and warns when missing', () => {
...
describe('policy-engine engine', () => {
  ...
  it('aggregates results deterministically and persists per-rule audit rows', async () => {
    ...
    expect(report.overallStatus).toBe('WARN'); // insurance missing => WARN
    expect(report.explainability).toEqual({ passCount: 2, warnCount: 1, failCount: 0 });
        ...
        "results": [
          {
            ...
            "result": {
              ...
              "reasons": [
                "No insurance credential provided",
              ],
              "status": "WARN",
